...
echo "=== Next Check Note ($(date)) ===" > "$(dirname "$0")/next_check_note.txt"
echo "Scheduled for: $MINUTES minutes" >> "$(dirname "$0")/next_check_note.txt"
echo "" >> "$(dirname "$0")/next_check_note.txt"
echo "$NOTE" >> "$(dirname "$0")/next_check_note.txt"
...
nohup bash -c "sleep $SECONDS && tmux send-keys -t $TARGET 'Time for orchestrator check! cat \"$(dirname \"$0\")/next_check_note.txt\" && python3 claude_control.py status detailed' && sleep 1 && tmux send-keys -t $TARGET Enter" > /dev/null 2>&1 &
