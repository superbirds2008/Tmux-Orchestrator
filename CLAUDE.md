...
## **Always run [self_test.md](self_test.md) to report envronment readyness at the begining of Tmux Orchestration**!!!
...
PROJECT_PATH="$HOME/Coding/$PROJECT_NAME"
...
# 2. Create session
tmux new-session -d -s task-templates -c "$HOME/Coding/task-templates"

# 3. Set up windows
tmux rename-window -t task-templates:0 "Claude-Agent"
tmux new-window -t task-templates -n "Shell" -c "$HOME/Coding/task-templates"
tmux new-window -t task-templates -n "Dev-Server" -c "$HOME/Coding/task-templates"
...
- **Location**: `$HOME/Coding/Tmux Orchestrator/send-claude-message.sh`
